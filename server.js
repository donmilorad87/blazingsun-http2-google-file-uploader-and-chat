...
const onRequest = (req, res) => {
  ...
  const reqPath = () => {
    if(req.headers[':path'] === '/'){
      ...
      return '/index.html'
    }else if(req.headers[':path'].toString().includes('/logged') || req.headers[':path'].toString().includes('/logged/')){
      return '/logged.html'
    }
    else{
  ...
  if (reqPath() === '/index.html') {
      ...
      push(res.stream, './public/style.css')
      push(res.stream, './public/jquery-3.5.1.min.js')
      push(res.stream, './public/main.js')
  }else if (reqPath().toString().includes('/logged') || reqPath().toString().includes('/logged/')) {

      push(res.stream, './public/style.css')
      push(res.stream, './public/logged.css')
      push(res.stream, './public/script.js')
      push(res.stream, './public/jquery-3.5.1.min.js')
      push(res.stream, './public/logged.js')
  }
